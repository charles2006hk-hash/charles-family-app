...
// --- Sub-Components (Defined OUTSIDE App to prevent ReferenceErrors and Re-renders) ---
      ...
      <div className="fixed bg-white p-3 rounded-lg shadow-xl border border-gray-200 w-64 pointer-events-none" style={style}>
        ...
        <div className="flex gap-1 mt-2">
          {event.participants?.map(pid => {
            ...
            return m ? <div key={pid} className="w-5 h-5 rounded-full text-[8px] flex items-center justify-center bg-gray-200">{m.name[0]}</div> : null;
          })}
        </div>
      </div>
    );
        ...
        return (
            <div className="h-screen flex items-center justify-center bg-gray-100 p-4">
                <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
                    <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center text-3xl font-bold mb-4 ${selectedMember.color}`}>{selectedMember.name[0]}</div>
...
const CalendarView = ({ currentDate, calendarView, events, trips, expenses, categories, members, setCalendarView, setCurrentDate, setHoveredEvent, openEventModal }) => {
    ...
    const header = (
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-4"><h2 className="text-xl font-bold">{year}年 {calendarView !== 'year' && `${month+1}月`}</h2><div className="flex bg-gray-100 rounded p-1">{['day','month','year'].map(v => <button key={v} onClick={() => setCalendarView(v)} className={`px-3 py-1 text-xs rounded capitalize ${calendarView===v?'bg-white shadow':''}`}>{v === 'day' ? '日' : v === 'month' ? '月' : '年'}</button>)}</div></div>
    ...
    if (calendarView === 'year') {
      ...
      return (
        <div className="bg-white rounded-lg shadow h-full flex flex-col">{header}
            ...
            {months.map(m => (
              <div key={m} className="border rounded p-2 hover:shadow-md cursor-pointer bg-white" onClick={() => { setCurrentDate(new Date(year, m, 1)); setCalendarView('month'); }}>
                ...
                <div className="grid grid-cols-7 gap-1 text-[8px] text-center text-gray-400">
                  ...
                  {Array.from({length: new Date(year, m+1, 0).getDate()}).map((_, i) => { const dStr = formatDate(new Date(year, m, i+1)); const isHol = HK_HOLIDAYS[dStr]; const hasTrip = trips.some(t => isDateInRange(dStr, t.startDate, t.endDate)); return <div key={i} className={`rounded-full h-5 w-5 flex items-center justify-center ${isHol ? 'bg-red-100 text-red-600 font-bold' : hasTrip ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100'}`}>{i+1}</div>; })}
                </div>
              </div>
            ))}
          </div>
    ...
    if (calendarView === 'day') {
      ...
      return (
        <div className="flex flex-col h-full bg-white rounded-lg shadow overflow-hidden">{header} 
           <div className="p-4 border-b flex justify-between items-center bg-gray-50"><h2 className="text-xl font-bold">{dStr} {HK_HOLIDAYS[dStr] ? `(${HK_HOLIDAYS[dStr]})` : ''}</h2><div className="text-sm text-gray-500">{getLunarInfo(currentDate).dayText} {getLunarInfo(currentDate).auspicious}</div></div>
           <div className="flex-1 overflow-y-auto relative">
             {hours.map(h => (
               <div key={h} className="flex border-b h-24 relative group">
                 ...
                 <div className="flex-1 relative p-1 hover:bg-blue-50/30 cursor-pointer" onClick={() => { openEventModal({ startTime: `${h}:00` }); }}>
                    {dayEvents.filter(e => parseInt(e.startTime) === h).map(ev => { const cat = categories.find(c => c.id === ev.type) || categories[0]; return (<div key={ev.id} onClick={(e) => { e.stopPropagation(); openEventModal(ev); }} className={`absolute left-2 right-2 rounded p-2 text-sm border-l-4 shadow-sm z-10 ${cat.color}`}><div className="font-bold flex justify-between"><span>{ev.title}</span><span className="opacity-75">{ev.startTime}-{ev.endTime}</span></div></div>); })}
                 </div>
               </div>
             ))}
           </div>
    ...
    return (
      <div className="bg-white rounded-lg shadow h-full flex flex-col">{header}<div className="grid grid-cols-7 border-b bg-gray-50 text-center py-1 text-sm text-gray-500"><div>日</div><div>一</div><div>二</div><div>三</div><div>四</div><div>五</div><div>六</div></div><div className="grid grid-cols-7 flex-1 overflow-y-auto">{days}</div></div>
    );
};

const ExpensesView = ({ expenses, openExpenseModal, handleToggleExpensePaid, deleteItem }) => {
...
const SettingsView = ({ currentUserRole, members, categories, handleLogout, setShowAddMemberModal, setTargetMemberId, setShowChangePasswordModal, handleUpdateCategory, setCategories }) => (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-8 overflow-y-auto">
      ...
      <section className="mb-8"><div className="flex justify-between items-center mb-4 border-b pb-2"><h3 className="font-bold text-gray-700">日曆項目分類 (Highlight)</h3></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{categories.map(c => (<div key={c.id} className={`p-3 rounded border flex justify-between items-center ${c.color}`}><div className="font-bold w-full">{c.name}</div>{c.type === 'custom' && (<div className="flex items-center gap-2"><button className="text-gray-400 hover:text-red-500" onClick={() => setCategories(categories.filter(x => x.id !== c.id))}><Trash2 size={14}/></button></div>)}</div>))}</div></section>
    </div>
);

const TravelView = ({ trips, openTripWizard, setShowPrintPreview, calculatePackingProgress, setEditingItem, setShowTripEditModal }) => (
...
const AddMemberModal = ({ isOpen, onClose, onAdd }) => {
    if(!isOpen) return null;
    const [name, setName] = useState(''); const [role, setRole] = useState('member');
    return (<div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4"><div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-sm"><h3 className="font-bold text-lg mb-4">新增家庭成員</h3><input className="w-full border p-2 rounded mb-4" placeholder="名稱 (例如: 爺爺)" value={name} onChange={e => setName(e.target.value)} /><select className="w-full border p-2 rounded mb-4" value={role} onChange={e => setRole(e.target.value)}><option value="member">一般成員</option><option value="admin">管理員</option></select><div className="text-xs text-gray-500 mb-4">預設密碼為 888888</div><div className="flex gap-2 justify-end"><button onClick={onClose} className="px-4 py-2 bg-gray-100 rounded">取消</button><button onClick={() => onAdd({name, role, color: 'bg-gray-100 text-gray-800'})} disabled={!name} className="px-4 py-2 bg-blue-600 text-white rounded font-bold">確認新增</button></div></div></div>);
};
...
export default function App() {
  ...
  // Render functions calling External Components
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-2 md:p-6 bg-gray-50">
            ...
            {activeTab === 'expenses' && <ExpensesView expenses={expenses} openExpenseModal={openExpenseModal} handleToggleExpensePaid={handleToggleExpensePaid} deleteItem={deleteItem} />}
            {activeTab === 'travel' && <TravelView trips={trips} openTripWizard={openTripWizard} setShowPrintPreview={setShowPrintPreview} calculatePackingProgress={calculatePackingProgress} setEditingItem={setEditingItem} setShowTripEditModal={setShowTripEditModal} />}
      ...
      <TripWizard isOpen={showTripWizard} onClose={() => setShowTripWizard(false)} onFinish={(d) => finishTripWizard(d, user, db, appId, setShowTripWizard)} members={members} />
      {showTripEditModal && editingItem && <PackingMode trip={editingItem} onClose={() => setShowTripEditModal(false)} onToggleItem={(t,c,i,u)=>{ const newList = JSON.parse(JSON.stringify(t.packingList)); if (c === 'shared') newList.shared[i].packed = !newList.shared[i].packed; else if (u) newList.individual[u][i].packed = !newList.individual[u][i].packed; updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'trips', t.id), { packingList: newList }); }} members={members} />}
