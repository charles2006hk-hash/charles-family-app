  ...
  const renderCalendar = () => {
    ...
    const header = (
        <div className="flex items-center justify-between p-4 border-b bg-white sticky top-0 z-10">
          <div className="flex items-center gap-4"><h2 className="text-xl font-bold">{year}年 {calendarView !== 'year' && `${month+1}月`} {calendarView === 'day' && `${month+1}月${currentDate.getDate()}日`}</h2><div className="flex bg-gray-100 rounded p-1">{['day','month','year'].map(v => <button key={v} onClick={() => setCalendarView(v)} className={`px-3 py-1 text-xs rounded capitalize ${calendarView===v?'bg-white shadow text-blue-600 font-bold':'text-gray-500'}`}>{v === 'day' ? '日' : v === 'month' ? '月' : '年'}</button>)}</div></div>
