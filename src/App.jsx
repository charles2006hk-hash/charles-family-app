...
// --- 3. View Components (DEFINED BEFORE APP TO AVOID REFERENCE ERRORS) ---
      ...
      <div className="fixed bg-white p-3 rounded-lg shadow-xl border border-gray-200 w-64 pointer-events-none" style={style}>
        ...
        <div className="flex gap-1 mt-2">
          {event.participants?.map(pid => {
            ...
            return m ? <div key={pid} className="w-5 h-5 rounded-full text-[8px] flex items-center justify-center bg-gray-200 overflow-hidden border border-white">
            ...
            <div className="h-screen flex items-center justify-center bg-gray-100 p-4">
                <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
                    <div className={`w-24 h-24 rounded-full mx-auto flex items-center justify-center text-5xl font-bold mb-4 ${selectedMember.color} overflow-hidden border-4 border-white shadow-md`}>
                        {(avatar.startsWith && avatar.startsWith('data:')) ? <img src={avatar} alt={selectedMember.name} className="w-full h-full object-cover"/> : avatar}
                    </div>
    ...
    return (
        <div className="h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
           <div className="text-center mb-10">
               <div className="w-24 h-24 mx-auto mb-4 rounded-3xl shadow-lg overflow-hidden bg-white flex items-center justify-center text-5xl bg-blue-500 text-white font-bold">C</div>
               <h1 className="text-3xl font-bold text-gray-800 mt-4">Charles Family App</h1>
...
const CalendarView = ({ currentDate, calendarView, events, trips, categories, members, setCalendarView, setCurrentDate, setHoveredEvent, openEventModal }) => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <div key={d} 
             ...
             className={`h-28 border-r border-b p-1 relative hover:bg-blue-50 transition-colors cursor-pointer ${isToday ? 'bg-blue-50' : 'bg-white'}`}>
  ...
  const renderExpenses = ({ expenses, openExpenseModal, handleToggleExpensePaid, deleteItem, currentUserRole }) => {
    // Permission Check
                                ...
                                <div key={item.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg group">
                                   ...
                                   <div className="text-right">
                                      ...
                                      <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                         <button onClick={() => openExpenseModal(item)} className="text-blue-500 hover:text-blue-700"><Edit2 size={12}/></button>
  ...
  const renderSettings = ({ currentUserRole, members, categories, handleLogout, setShowAddMemberModal, setTargetMemberId, setShowChangePasswordModal, handleUpdateCategory, setCategories }) => (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-8 overflow-y-auto">
      ...
      <section className="mb-8">
        ...
        <div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-100">
           <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center text-3xl overflow-hidden ${currentUserRole.color}`}>
                 {(currentUserRole.avatar && currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:')) ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : (currentUserRole.avatar || '👤')}
              </div>
              <div>
  ...
  const renderTravel = ({ trips, openTripWizard, setShowPrintPreview, calculatePackingProgress, setEditingItem, setShowTripEditModal, currentUserRole }) => {
    // Permission Check
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden bg-white border-b p-4 flex justify-between items-center z-20">
            ...
            {currentUserRole && <button onClick={() => setActiveTab('settings')} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${currentUserRole.color} overflow-hidden`}>{currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : (currentUserRole.avatar || '👤')}</button>}
        </header>

        <main className="flex-1 overflow-y-auto p-2 md:p-6 bg-gray-50">
            {activeTab === 'calendar' && renderCalendar()}
            {activeTab === 'expenses' && renderExpenses({ expenses, openExpenseModal, handleToggleExpensePaid, deleteItem, currentUserRole })}
            {activeTab === 'travel' && renderTravel({ trips, openTripWizard, setShowPrintPreview, calculatePackingProgress, setEditingItem, setShowTripEditModal, currentUserRole })}
            {activeTab === 'settings' && renderSettings({ currentUserRole, members, categories, handleLogout, setShowAddMemberModal, setTargetMemberId, setShowChangePasswordModal, handleUpdateCategory, setCategories })}
        </main>
