...
const INITIAL_EXPENSES = [
  { name: '大埔帝欣苑 (供款)', amount: 19038, day: 15, category: '樓宇', bank: 'DBS', type: 'recurring_monthly' }, { name: '大埔帝欣苑 (管理費)', amount: 2500, day: 15, category: '樓宇', bank: 'DBS', type: 'recurring_monthly' }
];
...
const getLunarInfo = (date) => {
  ...
  return { dayText: lunarDays[idx] || `${day}`, auspicious: (day % 15 === 0 || day === 1) ? '宜祭祀' : '' };
};
...
const CalendarView = ({ currentDate, calendarView, events, trips, categories, members, setCalendarView, setCurrentDate, setHoveredEvent, openEventModal }) => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      const dateObj = new Date(year, month, d); const dateStr = formatDate(dateObj); const isToday = formatDate(new Date()) === dateStr; const holiday = HK_HOLIDAYS[dateStr]; const lunar = getLunarInfo(dateObj); const dayEvents = events.filter(e => e.date === dateStr); const activeTrips = trips.filter(t => isDateInRange(dateStr, t.startDate, t.endDate));
      days.push(
        <div key={d} onClick={() => { setCurrentDate(dateObj); setCalendarView('day'); }} className={`h-28 border-r border-b p-1 relative hover:bg-blue-50 transition-colors cursor-pointer ${isToday ? 'bg-blue-50' : 'bg-white'}`}>
           <div className="flex justify-between items-start"><span className={`text-sm font-semibold w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-600 text-white' : 'text-gray-700'}`}>{d}</span><div className="flex flex-col items-end"><span className="text-[10px] text-gray-400">{lunar.dayText}</span>{holiday && <span className="text-[10px] text-red-500 font-bold">{holiday}</span>}</div></div>
           <div className="mt-1 flex flex-col gap-0.5 overflow-hidden h-[calc(100%-24px)]">
             ...
             {dayEvents.slice(0, 3).map(ev => { const cat = categories.find(c => c.id === ev.type) || categories[0]; return (<div key={ev.id} onMouseEnter={(e) => setHoveredEvent({ event: ev, x: e.clientX, y: e.clientY })} onMouseLeave={() => setHoveredEvent(null)} className={`text-[9px] px-1 rounded truncate border ${cat.color} cursor-help`}>{ev.title}</div>); })}
           </div>
        </div>
      );
    }
    ...
    return (
      <div className="bg-white rounded-lg shadow h-full flex flex-col">{header}<div className="grid grid-cols-7 border-b bg-gray-50 text-center py-2 text-sm font-bold text-gray-500"><div>日</div><div>一</div><div>二</div><div>三</div><div>四</div><div>五</div><div>六</div></div><div className="grid grid-cols-7 flex-1 overflow-y-auto bg-gray-100/50">{days}</div></div>
    );
};
...
const SettingsView = ({ currentUserRole, members, categories, handleLogout, setShowAddMemberModal, setTargetMemberId, setShowChangePasswordModal, handleUpdateCategory, setCategories }) => (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-8 overflow-y-auto">
      ...
      <section className="mb-8"><h3 className="font-bold text-gray-700 mb-4 border-b pb-2">當前登入</h3><div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-100"><div className="flex items-center gap-3"><div className={`w-12 h-12 rounded-full flex items-center justify-center text-3xl overflow-hidden ${currentUserRole.color}`}>{currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : currentUserRole.avatar}</div><div><div className="font-bold text-gray-800">{currentUserRole.name}</div><div className="text-xs text-gray-500">{currentUserRole.role === 'admin' ? '管理員' : '一般成員'}</div></div></div><button onClick={handleLogout} className="flex items-center gap-2 text-red-500 hover:text-red-700 text-sm font-bold bg-white px-4 py-2 rounded shadow-sm"><LogOut size={16}/> 登出</button></div></section>
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900">
      <div className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 z-10">
        ...
        <div className="p-4 border-t bg-gray-50">
          <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold border-2 border-white shadow overflow-hidden ${currentUserRole.color}`}>{currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : currentUserRole.avatar}</div><div className="truncate flex-1"><div className="font-bold text-sm">{currentUserRole.name}</div><div className="text-xs text-gray-500">{currentUserRole.role==='admin'?'管理員':'成員'}</div></div><button onClick={handleLogout} className="text-gray-400 hover:text-red-500"><LogOut size={16}/></button></div>
        </div>
      </div>
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden bg-white border-b p-4 flex justify-between items-center z-20"><h1 className="font-bold text-lg">Charles Family</h1>{currentUserRole && <button onClick={() => setActiveTab('settings')} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${currentUserRole.color} overflow-hidden`}>{currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : currentUserRole.avatar}</button>}</header>
