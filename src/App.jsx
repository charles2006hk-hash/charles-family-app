...
export default function App() {
  ...
  // --- Safe Avatar Helper (Defined inside App to ensure it's available) ---
              ...
              <div className="h-screen flex items-center justify-center bg-gray-100 p-4">
                  <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
                      <div className={`w-24 h-24 rounded-full mx-auto flex items-center justify-center text-5xl font-bold mb-4 ${loginTargetMember.color} overflow-hidden border-4 border-white shadow-md`}>
                          <SafeAvatar avatar={loginTargetMember.avatar} name={loginTargetMember.name} />
                      </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-2xl overflow-hidden border">
                            <SafeAvatar avatar={avatar} name="New" />
                        </div>
  ...
  const renderCalendar = () => {
    ...
    const header = (
        <div className="flex items-center justify-between p-4 border-b bg-white sticky top-0 z-10">
          <div className="flex items-center gap-4"><h2 className="text-xl font-bold">{year}年 {month+1}月 {calendarView === 'day' && `${currentDate.getDate()}日`}</h2><div className="flex bg-gray-100 rounded p-1">{['day','month','year'].map(v => <button key={v} onClick={() => setCalendarView(v)} className={`px-3 py-1 text-xs rounded capitalize ${calendarView===v?'bg-white shadow text-blue-600 font-bold':'text-gray-500'}`}>{v === 'day' ? '日' : v === 'month' ? '月' : '年'}</button>)}</div></div>
                                  ...
                                  <div className="flex items-center gap-1">
                                      {ev.participants?.map(p => {
                                          ...
                                          return mem ? <div key={p} className="w-5 h-5 rounded-full overflow-hidden bg-gray-100 border border-white text-[10px] flex items-center justify-center shadow-sm" title={mem.name}>
                                              <SafeAvatar avatar={mem.avatar} name={mem.name} />
                                          </div> : null
                                      })}
                                  </div>
                              </div>
        ...
        <div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-100">
           <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center text-3xl overflow-hidden ${currentUserRole.color}`}>
                 <SafeAvatar avatar={currentUserRole.avatar} name={currentUserRole.name}/>
              </div>
              <div>
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900">
      ...
      <div className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 z-10">
        ...
        <div className="p-4 border-t bg-gray-50">
          <div className="flex items-center gap-3">
             <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold border-2 border-white shadow overflow-hidden ${currentUserRole.color}`}><SafeAvatar avatar={currentUserRole.avatar} name={currentUserRole.name}/></div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden bg-white border-b p-4 flex justify-between items-center z-20">
            ...
            {currentUserRole && <button onClick={() => setActiveTab('settings')} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${currentUserRole.color} overflow-hidden`}><SafeAvatar avatar={currentUserRole.avatar} name={currentUserRole.name}/></button>}
        </header>
