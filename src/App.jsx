...
export default function App() {
  ...
  const handleAddMember = async (newMember) => {
      ...
      await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'members'), { ...newMember, password: '888888', createdAt: serverTimestamp() });
      setShowAddMemberModal(false);
      // setMemberFormData is handled in the modal component now for better isolation
  };
  ...
  const saveExpense = async (data) => {
    if (editingItem?.id) await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'expenses', editingItem.id), data);
  ...
  const finishTripWizard = (data) => {
      ...
      data.participants.forEach(pid => {
         ...
         if (pid.includes('dad')) individual[pid].push(createItem('鬚刨'));
         if (pid.includes('mom') || pid.includes('daughter')) individual[pid].push(createItem('化妝品'));
      });
              ...
              <div className="h-screen flex items-center justify-center bg-gray-100 p-4">
                  <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
                      <div className={`w-24 h-24 rounded-full mx-auto flex items-center justify-center text-5xl font-bold mb-4 ${loginTargetMember.color} overflow-hidden border-4 border-white shadow-md`}>
                          {loginTargetMember.avatar && loginTargetMember.avatar.startsWith('data:') ? <img src={loginTargetMember.avatar} alt={loginTargetMember.name} className="w-full h-full object-cover"/> : (loginTargetMember.avatar || '👤')}
                      </div>
                ...
                {members.map(m => (
                    ...
                    >
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-4xl font-bold ${m.color} overflow-hidden shadow-inner`}>
                            {m.avatar && m.avatar.startsWith('data:') ? <img src={m.avatar} alt={m.name} className="w-full h-full object-cover"/> : (m.avatar || '👤')}
                        </div>
  ...
  const renderCalendarHeader = () => (
    <div className="flex items-center justify-between p-4 border-b">
      <div className="flex items-center gap-4">
        <h2 className="text-xl font-bold">{currentDate.getFullYear()}年 {calendarView !== 'year' && `${currentDate.getMonth()+1}月`} {calendarView === 'day' && `${currentDate.getDate()}日`}</h2>
        ...
        <div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-100">
           <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center text-3xl overflow-hidden ${currentUserRole.color}`}>
                 {currentUserRole.avatar.startsWith && currentUserRole.avatar.startsWith('data:') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : currentUserRole.avatar}
              </div>
              <div>
                   ...
                   {trip.participants.map(pid => {
                     ...
                     return (
                       <div key={pid} className="flex items-center gap-2">
                         <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${m.color}`}>{m.avatar}</div>
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden bg-white border-b p-4 flex justify-between items-center z-20">
            ...
            {currentUserRole && <button onClick={() => setActiveTab('settings')} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${currentUserRole.color} overflow-hidden`}>{currentUserRole.avatar.startsWith('data:image') ? <img src={currentUserRole.avatar} className="w-full h-full object-cover"/> : currentUserRole.avatar}</button>}
        </header>
