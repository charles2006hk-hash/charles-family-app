...
import { 
  ...
  Printer, CheckSquare, Square, Weight, Palette, Home, Shield, Zap, DollarSign, Hotel, Bus, PieChart, TrendingUp, Wallet, Lock, LogOut, Key, Upload, Layout, X
} from 'lucide-react';
            ...
            <div className="h-screen flex items-center justify-center bg-gray-100 p-4">
                <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
                    <div className={`w-24 h-24 rounded-full mx-auto flex items-center justify-center text-5xl font-bold mb-4 ${selectedMember.color} overflow-hidden border-4 border-white shadow-md`}>
                        <SafeAvatar avatar={selectedMember.avatar} name={selectedMember.name} />
                    </div>
    ...
    return (
        <div className="h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
           <div className="text-center mb-10">
               <div className="w-24 h-24 mx-auto mb-4 rounded-3xl shadow-lg overflow-hidden bg-white flex items-center justify-center bg-blue-600 text-white">
                   <Layout size={48} />
               </div>
...
const CalendarView = ({ currentDate, calendarView, events, trips, categories, members, setCalendarView, setCurrentDate, setHoveredEvent, openEventModal }) => {
  ...
  const SettingsView = ({ currentUserRole, members, categories, handleLogout, setShowAddMemberModal, setTargetMemberId, setShowChangePasswordModal, handleUpdateCategory, setCategories }) => (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-8 overflow-y-auto">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{categories.map(c => (<div key={c.id} className={`p-3 rounded border flex justify-between items-center ${c.color}`}><div className="font-bold w-full">{c.name}</div>{c.type === 'custom' && (<div className="flex items-center gap-2"><button className="text-gray-400 hover:text-red-500" onClick={() => setCategories(categories.filter(x => x.id !== c.id))}><Trash2 size={14}/></button></div>)}</div>))}</div></section>
    </div>
  );
  ...
  const ExpenseFormModal = ({ isOpen, onClose, onSave, onDelete, initialData, historicalNames }) => {
    ...
    const handleNameChange = (e) => {
      ...
      // Simple suggestion logic could go here, but for now we just update name
    };
